...
const FAQ = [
  ...
  { q: "Süreler ne kadar güvenilir?",
    a: "M1A, M1B, M2, M3, M4, M5 ve M6 süreleri İBB Açık Veri GTFS tarifesinden, sefer sefer gerçek kalkış saatleriyle alınmıştır. M11 ve Marmaray'ın YOLCULUK SÜRELERİ TCDD tarifesinden türetilmiştir ve güvenilirdir; ancak bu iki hat için istasyon bazlı kalkış dakikası yayımlanmadığından bekleme ortalama olarak verilir; sıradaki trenin tam dakikası iddia edilmez. Yalnızca M7, M8, M9 ve Metrobüs için istasyon bazlı tarife bulunamadığı için süre mesafeye orantılı dağıtılmıştır; bu bacaklar sonuçta 'tahmini' olarak işaretlenir." },
  { q: "Aktarma süresi nasıl hesaplanıyor?",
    a: "Aktarma = peronlar arası yürüme + bekleme. Bekleme, gerçek tarifesi olan hatlarda sıradaki kalkışa göre dakikasıyla hesaplanır — bu yüzden aynı güzergâhta 5 dakika erken çıkmak varış saatini hiç değiştirmeyebilir ya da 1 dakika geç kalmak 15 dakika kaybettirebilir. Tarifesi olmayan hatlarda ortalama (sefer aralığı ÷ 2) kullanılır. Yürüme süresi hem yatay hem DİKEY mesafeyi kapsar: istasyon koordinatları arası mesafe (80 m/dk) + OpenStreetMap'ten alınan kat sayısı. Kat başına süre, istasyonda yürüyen merdiven varsa 0,8 dk, yoksa 1,3 dk alınır; iki kat ve fazlasında en az 3 dk uygulanır. Bu yüzden Mecidiyeköy (5 kat) 7-8 dk, Ayrılık Çeşmesi (2 kat, bitişik peron) 3 dk çıkar." },
...
(function(){
    ...
    legs.forEach(function(g, k){
      ...
      if(k > 0){
        ...
        for(var q=0;q<X.length;q++){
          var x = X[q];
          if((x[0]===prev.ln && x[2]===g.ln) || (x[2]===prev.ln && x[0]===g.ln)){ walk = x[4]; xlv = x[5] || 0; break; }
        }
      }
      var ready = t + walk;
      ...
      if(exact !== null){                   // gerçek tarife: dakikasıyla
        ...
        out.push({ ln: g.ln, from: N.n[g.i], to: N.n[g.j], iIdx: g.i, jIdx: g.j,
                   ...
                   fare: f0.tam, est: est0, exact: true, lv: xlv });
        t = arr0;
        return;
      }
  ...
  function render(plan, badge, cls, k){
    ...
    var rows = plan.legs.map(function(g){
      var pre = [];
      if(g.walk) pre.push(g.walk + " dk yürüme" + (g.lv ? " (" + g.lv + " kat)" : ""));
      pre.push(g.exact ? (g.wait + " dk bekleme") : ("~" + g.wait + " dk bekleme (ort.)"));
  ...
  function calc(){
    ...
    noteEl.innerHTML = (pf && pf.est
      ...
      "Bekleme, yayımlanmış tarifesi olan hatlarda (M1A–M6) sıradaki kalkışa göre dakikasıyla; M11, Marmaray, M7, M8, M9 ve Metrobüs'te ortalama (sefer aralığı ÷ 2) olarak hesaplanır — bu hatlar için istasyon bazlı kalkış dakikası yayımlanmıyor. " +
